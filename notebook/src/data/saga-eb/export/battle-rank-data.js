...
export const BattleRankData = () => {
  ...
  return (
    <StyledTableContainer align='center' sx={{ overscrollBehaviorY: 'contain' }}>
      <Table stickyHeader sx={{ width: `calc(52px + 64px * 6)`, tableLayout: 'fixed' }}>
        <TableHead sx={{ position: 'sticky', top: 0, zIndex: 3 }}>
          <TableRow>
            ...
            <StyledTh align='center' sx={{ width: '64px' }}>敵武器<br/>威力</StyledTh>
            <StyledTh align='center' sx={{ width: '64px' }}>敵SLv</StyledTh>
            <StyledTh align='center' sx={{ width: '64px' }}>基本<br/>属性耐性</StyledTh>
            <StyledTh align='center' sx={{ width: '64px' }}>HP<br/>補正率</StyledTh>
            <StyledTh align='center' sx={{ width: '64px' }}>基本<br/>能力値</StyledTh>
            <StyledTh align='center' sx={{ width: '64px' }}>成長<br/>ランク</StyledTh>
          </TableRow>
        </TableHead>
        <TableBody>
            ...
            if (Number(node.label.split('_')[1]) > 200) {
              return (
                <StyledTrTwoToneR className={index % 2 === 0 ? 'darker-row-w-header' : 'lighter-row-w-header'} key={index}>
                  <StyledTrh align='center' className='sticky' scope='row'>{Number(node.label.split('_')[1])}</StyledTrh>
