...
<Box>
  ...
  <StyledTableContainer align='center'>
    <Table stickyHeader sx={{ width: `calc(92px * 4)`, tableLayout: 'fixed' }}>
...
<Box>
  ...
  <StyledMathBox className='katex-box'>
    ...
    <BlockMath math='\hspace{0.5em}\text{ランク差評価値}=1+\text{潜在BR}\times \frac{\text{潜在BR}\times 1.5-\text{現在BR}}{\text{潜在BR}\times 10+\text{現在BR}\times 1.5}\hspace{0.5em}'/>
  </StyledMathBox>
...
<Box>
  ...
  <StyledMathBox className='katex-box'>
    <BlockMath
      math='\small\hspace{0.5em}\text{BR上限}=\begin{cases}\text{現在BR}+10+\text{ランク差ボーナス}&(\text{1.})\\
...
<Box>
  ...
  ここで、<InlineMath math={'\\small BP'}/> はバトルパーティを意味し、サポートメンバーや控えメンバーを含みません。

  メンバーの強さは下式で算出されます。

  <StyledMathBox className='katex-box'>
    <BlockMath math='\hspace{0.5em}\text{メンバーの強さ}=\text{HP評価値}+\text{能力値評価値}+\text{スキル評価値}\hspace{0.5em}'/>
    <BlockMath math='\hspace{0.5em}\begin{aligned}\text{HP評価値}&=\frac{(HP-140)\times 18}{1000}\hspace{0.5em}\\[0.89em]
        \hspace{0.5em}\text{能力値評価値}&=\frac{(\max Stat-20)\times 72}{1000}\hspace{0.5em}\\[0.89em]
        \hspace{0.5em}\text{スキル評価値}&=\frac{(\max SLv-5)\times 315}{1000}\end{aligned}\hspace{0.5em}'/>
  </StyledMathBox>
