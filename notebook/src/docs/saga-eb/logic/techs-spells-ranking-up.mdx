...
<Box>
  ...
  <StyledMathBox className='katex-box'>
    <BlockMath
      math='\small\hspace{0.5em}\text{使用回数}_{next}=\text{使用回数}+\text{使用回数加算量}\hspace{0.5em}'
    />
  </StyledMathBox>
  ...
  技ランク成長アップの有無と成長ボーナス発生確率の関係は下表の通りです。

  <StyledTableContainer align='center'>
    <Table stickyHeader sx={{ width: `calc(92px * 2)` }}>
      <TableHead>
        <TableRow>
...
<Box>
  ...
  <StyledTableContainer align='center'>
    <Table stickyHeader sx={{ width: `calc(140px + 128px * 2 + 160px)` }}>
      <TableHead sx={{ position: 'sticky', top: 0, zIndex: 3 }}>
        <TableRow>
          <StyledTh align='center' rowSpan={2} sx={{ position: 'sticky', left: 0, zIndex: 3, width: '140px' }}>使用回数が<br/>修正されるケース</StyledTh>
          <StyledTh align='center' colSpan={2}>修正対象</StyledTh>
          <StyledTh align='center' rowSpan={2} sx={{ width: '160px' }}>修正後の使用回数</StyledTh>
        </TableRow>
        <TableRow>
          <StyledTh align='center' sx={{ width: '128px' }}>味方キャラクター</StyledTh>
          <StyledTh align='center' sx={{ width: '128px' }}>技・術</StyledTh>
        </TableRow>
      </TableHead>
      <TableBody>
